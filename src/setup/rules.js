...
export function rulesBlock({ mcp = false } = {}) {
  const commands = mcp
    ...
    : [
        '| Command | |',
        '|---|---|',
        '| `npx tailr start --target <url>` | start a session (detached); exit 0 when the review URL is ready |',
        '| `npx tailr stop` | stop the session |',
        ...
        '| `npx tailr config [name:value…]` | their settings — `sfx`, `modifier`, `tutorial`. Only when they ask |'
      ];
