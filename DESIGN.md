...
- **The page states what the action will do before it asks for it.** Under the pill, in plain text and never as cards or tiles, it carries the two facts the README makes and the visitor is being asked to take on faith: that Tailr proxies the dev server rather than modifying it, and that a session writes nothing to the repository but `.tailr/session.json`. A page that asks a stranger to hand an agent a fetch-and-obey instruction and says nothing about its blast radius has skipped the only question that visitor has.
