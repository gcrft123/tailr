...
- **The page states what the action will do before it asks for it.** Under the pill, in plain text and never as cards or tiles, it carries the two facts the README makes and the visitor is being asked to take on faith: that Tailr proxies the dev server rather than modifying it, and that a session writes nothing to the repository but a locator under `.tailr/`. A page that asks a stranger to hand an agent a fetch-and-obey instruction and says nothing about its blast radius has skipped the only question that visitor has.
- **A showcase must be true at rest, not only mid-animation.** Each replay plays once and then holds, and the frame it holds is the one most visitors actually study — it is also what reduced motion and a scroll-back give them. So the held state carries the showcase's own claim: the inline edit rests on the edited word, and the versions showcase rests on its version tabs rather than handing the slot to the slider, because the heading promises options.
- The page carries **one headline action**, an ink pill in the hero, and the run of showcases never asks for it again. It is answered once at the foot of the page by the agent fan, which is a different offer rather than a repeat of the same one: the hero pill hands over the prompt for an agent to follow, and each disc hands over the exact setup line for one named host. Between them sit only the per-showcase replays. Nothing else on the page is interactive, and no third ask is added.
- **The fan is bound to the README, not to a wish list.** A disc exists only for a host the README documents a route for, and it copies that route's text verbatim — the marketplace command for Claude Code, Codex, GitHub Copilot and Gemini CLI, the marketplace URL for Cursor (which has no command), and `npx skills add` for the hosts that read a global skills directory. A brand with no documented route does not get a disc. The fan tightens on small screens by pulling the discs in, never by shrinking them: the spread scales, the 52px disc does not, so the target stays the same size under a thumb as under a cursor. Where there is no hover to read a disc by, the first press opens its label and the second one copies.
- `paper` never appears in the page's own chrome at all — on this surface it exists solely inside the staged Island replicas, where it means what it means everywhere else in Tailr. A blocked clipboard reveals the text itself on the page, selectable, rather than sending the visitor somewhere: under the pill for the hero prompt, under the fan for a host's setup line. Both results are also announced in the page's one live region, because a copy result is a label swap and a label swap is not read.
- **Focus on this page is an ink ring with a white halo, not the overlay's `live` teal.** The teal ring is correct everywhere it was designed for, because the island is always ink; on a white page it measures 1.89:1 against its own background and fails the 3:1 floor on nearly every stop. So the page reuses the move its on-page marks already make for unknown ground — a paired stroke — and flips both halves per region: ink line with a white halo on the white run, white line with an ink halo on the footer. A control that is round takes a round ring. Nothing focusable may be invisible while focused: the per-showcase replay is hidden until its stage has played and comes back for keyboard focus.
