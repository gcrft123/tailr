...
- A mark whose element is no longer on screen is **hidden, never silently re-anchored and never discarded**. It keeps its number, its comment and its last known address; what it loses is somewhere on the page to pin a badge to. Re-anchoring risks attaching a comment to the wrong element and having the agent act on it; discarding breaks the guarantee that staged markup is never lost.
- **Hidden is a display state, and it reaches the agent as nothing at all.** The agent works in the source, where an element that stopped rendering and one that never moved are the same mark, and the address still resolves. Flagging it would stop the agent to ask about work it could have done. The reviewer is told, because the missing badge is theirs to explain; the agent is not, because there is nothing to tell them.
- A hidden mark stays **fully editable** from the staged list, which is where the panel points the reviewer. Its composer opens in the middle of the viewport rather than pinned to an element that is not there. Inline text edits are the exception: the reviewer types into the element, so there has to be one.
- Calling a mark hidden is a claim about the element, so it may only be made from the route the mark was made on, and only once that route has had a chance to render. A mark belonging to a page the reviewer has since navigated away from is **not** hidden — it is unverifiable, and stays staged under its own route until they return to it. A route-shaped hash (`#/orders`) is part of that address; a plain anchor (`#pricing`) is not.
...
A comment on an element, or on a spot, can ask for **up to four versions of the same change**, built together so the reviewer compares them on the running page rather than in prose. The same mark can also ask for a **continuous slider** — a numerical parameter the reviewer scrubs on the page (glow intensity, bevel depth, geometry). Both are the places where the agent's work is reflected back into the overlay beyond the reload prompt.
...
- A slider is one number behind `data-tailr-slide-<ref>`, with a range the agent reports. The default value is what renders when the attribute is absent.
...
- **The guards are scaffolding, and they live for exactly one round trip.** Keeping a version or a slider value is itself a mark: it goes into the next batch, and it is what takes the losing versions or the switch out of the source. A reviewer must never be able to reach a state where versions or sliders are staged in their repository with nothing in Tailr that would remove them — which is why an unresolved set keeps the island awake, and why turning the whole set down is an offered action rather than something achieved by ignoring it.
- Removals and inline text edits have no versions or sliders. A deletion has one outcome, and an inline edit is the reviewer writing the answer themselves.
